...
export function TierSettingsDialog({ open, onClose }: TierSettingsDialogProps) {
  const { tiers, renameTier, setTierColor, addTier, removeTier } = useTierStore();
              ...
              <div className="flex flex-col gap-0.5 shrink-0 w-20">
                <textarea
                  ...
                  onBlur={() => {
                    ...
                    if (!val) setLabelErrors((prev) => ({ ...prev, [tier.id]: true }));
                  }}
                ...
                />
                {labelErrors[tier.id] && (
                  <p className="text-[10px] text-destructive whitespace-nowrap">ใส่อย่างน้อย 1 ตัวอักษร</p>
                )}
              </div>
                ...
                {PRESET_COLORS.map((c) => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-sm border transition-transform hover:scale-110 ${
                      tier.color === c ? 'border-foreground ring-2 ring-foreground/30 scale-110' : 'border-border/40'
                    }`}
