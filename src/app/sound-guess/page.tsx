...
import { serializePublicSoundGuessGameSummary } from "@/lib/sound-guess-public";
import { getPublicSoundGuessGames } from "@/services/sound-guess-games.service";
import { SoundGuessGalleryClient } from "./_components/sound-guess-gallery-client";

const HOME_URL = "https://mavelus-jk.com";

export const metadata: Metadata = {
  title: "Sound Guess | Public Games",
  description: "เลือกเกมทายเสียงแบบสาธารณะแล้วเริ่มเล่นได้ทันที",
};

/**
 * Renders the public Sound Guess gallery page.
 *
 * @returns Server-rendered public gallery with client-side search.
 */
export default async function SoundGuessPage() {
  await connection();

  const games = (await getPublicSoundGuessGames()).map(
    serializePublicSoundGuessGameSummary,
  );
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="space-y-3">
              <h1 className="max-w-3xl text-4xl font-bold tracking-tight text-foreground md:text-5xl">
                เลือกเกมทายเสียงที่อยากท้าทาย
              </h1>
              <p className="max-w-3xl text-base leading-7 text-muted-foreground md:text-lg">
                ให้ host เปิดเสียงทีละข้อ ซ่อนคำตอบไว้ก่อน แล้วเฉลยเพื่อเก็บคะแนนให้ผู้ตอบถูก
              </p>
            </div>
          </div>
        </header>

        <SoundGuessGalleryClient games={games} />
      </section>
    </div>
  );
}
