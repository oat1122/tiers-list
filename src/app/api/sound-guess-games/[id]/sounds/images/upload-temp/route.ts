...
import { saveSoundGuessTempSoundImageFile } from "@/lib/sound-guess-upload";
import {
...
/**
 * Uploads a temporary cropped image for an admin sound guess sound item.
 *
 * @param request - Admin multipart request containing the image file.
 * @param props - Next route context containing the async game id params.
 * @returns JSON response containing the temporary sound image upload path.
 */
export async function POST(
  ...
  try {
    ...
    const tempImagePath = await saveSoundGuessTempSoundImageFile(image);
    return NextResponse.json({ tempImagePath }, { status: 201 });
  } catch (error) {
