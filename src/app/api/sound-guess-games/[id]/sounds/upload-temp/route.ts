...
import { saveSoundGuessTempAudioFile } from "@/lib/sound-guess-upload";
import {
...
/**
 * Uploads a temporary audio file for an admin sound guess game.
 *
 * @param request - Admin multipart request containing the audio file.
 * @param props - Next route context containing the async game id params.
 * @returns JSON response containing the temporary audio upload path.
 */
export async function POST(
  ...
  try {
    ...
    const tempAudioPath = await saveSoundGuessTempAudioFile(audio);
    return NextResponse.json({ tempAudioPath }, { status: 201 });
  } catch (error) {
